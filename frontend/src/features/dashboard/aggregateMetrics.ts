...
function pctFromSeconds(d: Record<string, number>): Record<string, number> {
  const total = Object.values(d).reduce((a, b) => a + b, 0);
  if (total <= 0) return {};
  const out: Record<string, number> = {};
  for (const [k, v] of Object.entries(d)) out[k] = Math.round((1000 * v) / total) / 10;
  return out;
}
...
export function aggregateMetrics(summaries: MetricsSummary[]): MetricsSummary {
  const activity_seconds: Record<string, number> = {};
  const rollup_seconds: Record<string, number> = {};
