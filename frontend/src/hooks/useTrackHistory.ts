...
import type { CameraStateData, TrackEntry } from "./useEventsWS";

export type TrackSample = {
  t: number;
  rollup: TrackEntry["rollup"];
 ...
 */
export function useTrackHistory(state: CameraStateData | null): Map<number, TrackHistory> {
  const bufferRef = useRef<Map<number, TrackHistory>>(new Map());
  const [snapshot, setSnapshot] = useState<Map<number, TrackHistory>>(new Map());
    ...
    if (!pendingRef.current) {
      ...
      requestAnimationFrame(() => {
        ...
        const sorted = new Map(
          [...bufferRef.current.entries()].sort((a, b) => a[0] - b[0])
        );
        setSnapshot(sorted);
      });
    }
